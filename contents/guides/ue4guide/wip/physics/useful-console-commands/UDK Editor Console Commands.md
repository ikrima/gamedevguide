...
-   **CREATE\_BV\_BOUNDINGBOX \[SNAPTOGRID=snap\]** – Creates a blocking volume from the bounding box of the selected static meshes with optional snapping.
    -   **CREATE\_BV\_CONVEXVOLUME \[SNAPTOGRID=snap\] \[NORMALTOLERANCE=tolerance\] \[NLIMITTX=limitx\] \[NLIMITY=limity\] \[NLIMITZ=limitz\]** – Creates a blocking volume from a convex volume containing the selected static meshes using the given normal tolerance and limits with optional snapping.
